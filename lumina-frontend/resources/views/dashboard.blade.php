    ...
    async function uploadImage() {
      ...
      if (fileInput.files.length === 0) {
        await Swal.fire({
          ...
          text: 'Pilih gambar terlebih dahulu sebelum menganalisis!',
        });
        return;
      }
      ...
      if (!user) {
        await Swal.fire({
          icon: 'error',
          title: 'Belum login',
          text: 'Kamu harus login dulu untuk menggunakan fitur ini.',
        });
        return;
      }

      const confirmResult = await Swal.fire({
        title: 'Analisis Sekarang?',
        text: "Apakah kamu yakin ingin menganalisis gambar ini?",
        icon: 'question',
      ...
      try {
        ...
        if (response.ok) {
          ...
          await Swal.fire({
            icon: 'error',
            title: 'Gagal',
            text: 'Gagal mengupload & menganalisis gambar.',
          });
        ...
        await Swal.fire({
          ...
          text: 'Ada sesuatu yang salah saat menganalisis gambar.',
        });
        loading.style.display = 'none';
      }
    }
